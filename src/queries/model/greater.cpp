...
void GreaterModel::selectChunks() {
    if (hasFunctionInside) {
        ...
        for (int i =0 ; i< m; i++) {
            ...
            if (st != ChunkStateEmpty && indexManagers.at(0)->compareGreater(i,toFilter,isaFloat)) {
                jobs.push_back(i);
            }
        }
...
uint64_t GreaterModel::execShaderThreadOnCpu(uint32_t thread,uint32_t nbThreads) {
...
void GreaterModel::execute(queries::builders::AST *ast) {
    executeQuery(ast);
}
