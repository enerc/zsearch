...
namespace floaxie
{
	 ...
	 * \see `max_buffer_size()`
	 */
	template<typename FloatType, typename CharType> inline int ftoa(FloatType v, CharType* buffer) noexcept
	{
		if (std::isnan(v))
		{
			buffer[0] = 'n';
		...
		else
		{
			*buffer = '-';
			...
			return prettify<decimal_scientific_threshold>(buffer, len, K);
		}
	}
