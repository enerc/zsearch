...
%parse-param { zsql::SQLParserResult* result }
...
%token NOT OFF SET TOP AS BY IF IN IS OF ON OR XOR TO
%token ARRAY CONCAT ILIKE SECOND MINUTE HOUR DAY MONTH YEAR
%token TRUE FALSE
%token TRANSACTION BEGIN COMMIT ROLLBACK
%token MULTILINESTRING_SYM MULTIPOINT_SYM MULTIPOLYGON_SYM POINT_SYM POLYGON_SYM
%token POINT ST_SRID ST_DISTANCE_SPHERE COUNT MIN MAX SUM AVG
...
%type <expr>		    point_expr srid_expr distance_expr count_expr min_expr max_expr sum_expr avg_expr
...
%left		'*' '/' '%'
%left		'^'
%left		CONCAT ST_DISTANCE_SPHERE
...
file_type:
		IDENTIFIER {
			if (strcasecmp($1, "json") == 0) {
				$$ = kImportJSON;
			} else if (strcasecmp($1, "lz4") == 0) {
				$$ = kImportLZ4;
			} else {
				free($1);
...
logic_expr:
	...
	|	expr XOR expr	{ $$ = Expr::makeOpBinary($1, kOpXor, $3); }
	;
...
point_expr:
	  	POINT '(' expr_list ')' { $$ = Expr::makePoint($3); }
	 ;

srid_expr:
	  	ST_SRID '(' point_expr ',' int_literal ')' { $$ = Expr::makeSrid($3,$5->ival); }
	;

count_expr:
	  	COUNT '(' column_name ')' { $$ = Expr::makeCount($3); }
	;

min_expr:
	  	MIN '(' column_name ')' { $$ = Expr::makeMin($3); }
	;

max_expr:
	  	MAX '(' column_name ')' { $$ = Expr::makeMax($3); }
	;

sum_expr:
	  	SUM '(' column_name ')' { $$ = Expr::makeSum($3); }
	;

avg_expr:
	  	AVG '(' column_name ')' { $$ = Expr::makeAvg($3); }
	;

distance_expr:
	 	ST_DISTANCE_SPHERE '(' column_name ','  srid_expr ')'  { $$ = Expr::makeDistanceSphere($3,$5); }
	;
