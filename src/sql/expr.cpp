...
__int128_t parseSigned128(const char *src) {
    string_view v(src,strlen(src));
    return parseSigned128(v,128);
}
namespace zsql {
...
Expr* Expr::makeLiteral(__int128_t val) {
...
Expr *Expr::makePoint(std::vector<Expr*> *exprList) {
    Expr* e = new Expr(kExprOperator);
    e->opType = kOpPoint;
    e->exprList = exprList;
    return e;
...
const char* makeSridName = "ST_SRID";
Expr *Expr::makeSrid(Expr *expr,__int128 spatialReferenceSystemID) {
    Expr* e = new Expr(kExprSrid);
    e->opType = kOpSrid;
    e->ival = spatialReferenceSystemID;
    e->exprList = new std::vector<Expr*>();
    e->exprList->push_back(expr);
    e->name = strdup(makeSridName);
    return e;
}

const char* makeDistanceSphereName = "ST_Distance_Sphere";
Expr *Expr::makeDistanceSphere(Expr* expr,Expr*expr2) {
    Expr* e = new Expr(kExprDistanceSphere);
    e->opType = kOpDistanceSphere;
    e->exprList = new std::vector<Expr*>();
    e->exprList->push_back(expr);
    e->exprList->push_back(expr2);
    e->name = strdup(makeDistanceSphereName);
    return e;
}

const char* makeCountName = "Count";
Expr *Expr::makeCount(Expr* expr) {
    Expr* e = new Expr(kExprFunctionRef);
    e->opType = kOpCount;
    e->exprList = new std::vector<Expr*>();
    e->exprList->push_back(expr);
    e->name = strdup(makeCountName);
    return e;
}

const char* makeMinName = "Min";
Expr *Expr::makeMin(Expr* expr) {
    Expr* e = new Expr(kExprFunctionRef);
    e->opType = kOpMin;
    e->exprList = new std::vector<Expr*>();
    e->exprList->push_back(expr);
    e->name = strdup(makeMinName);
    return e;
}

const char* makeMaxName = "Max";
Expr *Expr::makeMax(Expr* expr) {
    Expr* e = new Expr(kExprFunctionRef);
    e->opType = kOpMax;
    e->exprList = new std::vector<Expr*>();
    e->exprList->push_back(expr);
    e->name = strdup(makeMaxName);
    return e;
}
const char* makeSumName = "Sum";
Expr *Expr::makeSum(Expr* expr) {
    Expr* e = new Expr(kExprFunctionRef);
    e->opType = kOpSum;
    e->exprList = new std::vector<Expr*>();
    e->exprList->push_back(expr);
    e->name = strdup(makeSumName);
    return e;
}
const char* makeAvgName = "Avg";
Expr *Expr::makeAvg(Expr* expr) {
    Expr* e = new Expr(kExprFunctionRef);
    e->opType = kOpAvg;
    e->exprList = new std::vector<Expr*>();
    e->exprList->push_back(expr);
    e->name = strdup(makeAvgName);
    return e;
}
